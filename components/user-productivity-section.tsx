...
export function UserProductivitySection() {
  ...
  const { data: userProductivity = [], isLoading } = useUserProductivity(); // Fetch department summary
  const users = userProductivity;
  ...
  const totalTasksToday = filteredAndSortedUsers.reduce(
    (sum, user) => sum + user.tasks_completed_today,
    0
  );
  const totalTasksWeek = filteredAndSortedUsers.reduce(
    (sum, user) => sum + user.tasks_completed_week,
    0
  );
    ...
    filteredAndSortedUsers.length > 0
      ? Math.round(
          filteredAndSortedUsers.reduce(
            (sum, user) => sum + user.productivity_score,
            0
        ...
        <Card className='border-0 shadow-lg bg-gradient-to-br from-white to-blue-50 hover:shadow-xl transition-all duration-300'>
          ...
          <CardContent>
            ...
            <p className='text-xs text-slate-600'>
              From {filteredAndSortedUsers.length} users
            </p>
          </CardContent>
        </Card>
                  ...
                  <TableRow key={user.user_id} className='hover:bg-slate-50/50'>
                    ...
                    <TableCell className='min-w-[120px]'>
                      <div className='space-y-1'>
                        ...
                        <Progress
                          value={user.avg_completion_rate}
                          className='h-2'
                        />
                      </div>
                    </TableCell>
